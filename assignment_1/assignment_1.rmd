...
0 & 0.9 & 0.1 \\
0 & 0 & 1 \end{bmatrix} = \begin{bmatrix} 0.75 & 0.14 & 0.11 \end{bmatrix}\end{equation}


\begin{equation}\implies \begin{bmatrix}  0.75 & 0.14 & 0.11 \end{bmatrix} \begin{bmatrix}  0.75 & 0.14 & 0.11 \\
0 & 0.9 & 0.1 \\
 0 & 0 & 1 \end{bmatrix} = \begin{bmatrix} 0.5625 & 0.231 & 0.2065 \end{bmatrix}\end{equation}

\begin{equation}\implies \begin{bmatrix}  0.5625 & 0.231 & 0.2065 \end{bmatrix} \begin{bmatrix}  0.75 & 0.14 & 0.11 \\
0 & 0.9 & 0.1 \\
 0 & 0 & 1 \end{bmatrix} = \begin{bmatrix} 0.421875 & 0.28665 & 0.291475 \end{bmatrix}\end{equation}
...
\[\begin{bmatrix} 1 & 0 & 0 \end{bmatrix} \begin{bmatrix}  0.75 & 0.14 & 0.11 \\
0 & 0.9 & 0.1 \\
...
\color{red}
Assuming the only probability that is unknown is the probability from WELL to ILL (and that all other transition probability are the same as  (a)-(c)), we can deduce by looking at cycle 0 to cycle 1, that the probability of transitioning from WELL to ILL is $21\%$. That means that the transition state matrix is:


\[\begin{bmatrix} 0.75 & 0.21 & 0.11 \\
0 & 0.9 & 0.1 \\
 0 & 0 & 1 \\\end{bmatrix} \hspace{1in} \begin{matrix} 
\text{} & \text{Well} & \text{Ill}  & \text{Death}\\
\text{Well} & 0.75 & 0.21 & 0.11 \\
\text{Ill} & 0 & 0.9 & 0.1 \\
\text{Death} & 0 & 0 & 1 \\
\end{matrix}\]
...
probability_matrix <- matrix(c(0.75, 0.21, 0.11, 
                               0, 0.9, 0.1, 
                               0, 0, 1), nrow = 3, byrow = TRUE)
...
\color{red}
Factors that could change the result noted in (e) could be if there was a smaller utility associated with the ILL state. Since new prophylaxis (Tracing A) allows people to transition from ILL to WELL, if the reward gain from WELL was higher that might allow new prophylaxis (Tracing A) to be the preferred strategy. Alson since the new treatment (Tracing B) has a greater fraction of its lifetime cycle reward deriving from ILL people's utility, if that utility weight were smaller it would disproportinately effect the sum of the lifetime reward for Tracing B compared to Tracing A.
\color{black}
